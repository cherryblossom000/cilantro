...
const command = {
    ...
    async execute(interaction, player) {
        ...
        const progress = Math.round((currentTime / nowPlaying.milliseconds) * BAR_LENGTH);
        await interaction.reply({
            ...
            content: codeBlock(`${'█'.repeat(progress)}${'▒'.repeat(BAR_LENGTH - progress)} [${Utils.msToTime(currentTime)}/${nowPlaying.duration}]`)
        });
    }
};
