...
const command = {
    data: new SlashCommandBuilder()
        ...
        .addChoices(...Object.entries(uploadDateChoices).map(([name, value]) => ({
        name,
        value
    }))))
        .addStringOption(option => option
        ...
        .addChoices(...Object.entries(durationDateChoices).map(([name, value]) => ({
        name,
        value
    }))))
        .addStringOption(option => option
        ...
        .addChoices(...Object.entries(sortChoices).map(([name, value]) => ({
        name,
        value
    })))),
    execute: async (interaction, player, guildsToTextChannels, db) => {
        ...
        const song = await Utils.best(query, {
            timecode: true,
            requestedBy: user,
            ...
            duration: (options.getString(DURATION) ??
                undefined),
            sortBy: (options.getString(SORT) ?? undefined)
        }, queue);
        ...
        try {
            ...
            if (error instanceof DMPError && error.name === DMPErrors.SEARCH_NULL) {
                await interaction.reply(`No YouTube search results found for ${bold(query)}.`);
                return;
            }
            throw error;
        }
        await setChannel(guildsToTextChannels, interaction);
        await interaction.editReply(wasPlaying
