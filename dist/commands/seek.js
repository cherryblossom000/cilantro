...
const TIME = 'time';
const HELP_MSG = `${inlineCode(TIME)} must be a timestamp, with optionally a ${inlineCode('+')} or ${inlineCode('-')} to indicate times relative to the current position in the song.
Examples:
...
const command = {
    data: new SlashCommandBuilder()
        ...
        .setName(TIME)
        .setDescription('A timestamp with optional + or - for relative times (e.g. +10, -1:00).')
        .setRequired(true)),
    async execute(interaction, player, guildsToTextChannels) {
        ...
        await interaction.editReply(`Playing from ${Utils.msToTime(ms)}. This might take some time…`);
    }
};
