...
const toError = (reason) => reason instanceof Error ? reason : new Error(`${reason}`);
if (dev) {
    ...
    process.on('unhandledRejection', reason => handleError(client, 'Uncaught promise rejection')(toError(reason)));
    process.on('uncaughtException', handleError(client, 'Uncaught exception'));
}
const guildsToTextChannels = new Map();
const player = new Player(client, { deafenOnJoin: true })
    .on('error', (error, queue) => handleError(client, `The ${inlineCode('error')} player event fired for queue for guild ${queue.guild.id}`)(error))
    .on('songChanged', ({ guild }, song) => {
    const channel = guildsToTextChannels.get(guild.id);
    channel
        ? channel
            .send(nowPlayingText(song))
            .catch(handleError(client, `Sending now playing message to channel ${channel.id} failed`))
        : console.error(`Guild ${guild.id} does not have a text channel`);
});
client
    .once('ready', () => console.log('ready'))
    .on('error', handleError(client, `The ${inlineCode('error')} client event fired`))
    ...
    if (interaction.isCommand()) {
        ...
        await execute(interaction, player, guildsToTextChannels, database).catch(handleError(client, `Failed to execute command ${commandName}`, interaction, `Sorry, there was an error trying to execute /${commandName}.`));
    }
});
await client.login();
